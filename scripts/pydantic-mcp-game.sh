...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    ...
    echo ""
    echo "Usage: ./scripts/pydantic-mcp-game.sh [difficulty] [model] [delay] [max_turns]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./scripts/pydantic-mcp-game.sh full google-gla:gemini-3-flash-preview 1 25"
    exit 0
fi
...
uv run python3 scripts/pydantic_mcp_client.py "$LEVEL" "$MODEL" "$DELAY" "$MAX_TURNS"
