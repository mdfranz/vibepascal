...
usage() {
    ...
    echo ""
    echo "Usage: ./agno-mcp-game.sh <model> <max_turns> [delay] [difficulty]"
    echo ""
    echo "Arguments:"
    echo "  model         OpenAI model name (required)"
    ...
    echo ""
    echo "Examples:"
    echo "  ./agno-mcp-game.sh gpt-5 25 1 full"
    exit 1
}
...
uv run --project packages/agno python3 packages/agno/agno_mcp_client.py "$LEVEL" "$MODEL" "$DELAY" "$MAX_TURNS"
