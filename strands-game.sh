...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    ...
    echo ""
    echo "Usage: ./strands-game.sh [difficulty] [model] [delay] [max_turns]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./strands-game.sh full gpt-4o-mini 1 25"
    exit 0
fi
...
uv run --project packages/strands python3 packages/strands/strands_client.py "$LEVEL" "$MODEL" "$DELAY" "$MAX_TURNS"
